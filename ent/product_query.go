...
type ProductQuery struct {
	...
	withBarcodes       *BarcodeQuery
	withPriceHistories *PriceHistoryQuery
...
// QueryBarcodes chains the current query on the "barcodes" edge.
func (pq *ProductQuery) QueryBarcodes() *BarcodeQuery {
	query := (&BarcodeClient{config: pq.config}).Query()
...
// QueryPriceHistories chains the current query on the "price_histories" edge.
func (pq *ProductQuery) QueryPriceHistories() *PriceHistoryQuery {
	query := (&PriceHistoryClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withBarcodes:       pq.withBarcodes.Clone(),
		withPriceHistories: pq.withPriceHistories.Clone(),
...
// WithBarcodes tells the query-builder to eager-load the nodes that are connected to
// the "barcodes" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithBarcodes(opts ...func(*BarcodeQuery)) *ProductQuery {
	query := (&BarcodeClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withBarcodes = query
	return pq
}

// WithPriceHistories tells the query-builder to eager-load the nodes that are connected to
// the "price_histories" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithPriceHistories(opts ...func(*PriceHistoryQuery)) *ProductQuery {
	query := (&PriceHistoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPriceHistories = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withBarcodes; query != nil {
		if err := pq.loadBarcodes(ctx, query, nodes,
			func(n *Product) { n.Edges.Barcodes = []*Barcode{} },
			func(n *Product, e *Barcode) { n.Edges.Barcodes = append(n.Edges.Barcodes, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withPriceHistories; query != nil {
		if err := pq.loadPriceHistories(ctx, query, nodes,
			func(n *Product) { n.Edges.PriceHistories = []*PriceHistory{} },
			func(n *Product, e *PriceHistory) { n.Edges.PriceHistories = append(n.Edges.PriceHistories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadBarcodes(ctx context.Context, query *BarcodeQuery, nodes []*Product, init func(*Product), assign func(*Product, *Barcode)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(barcode.FieldProductID)
	}
	query.Where(predicate.Barcode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.BarcodesColumn), fks...))
	}))
...
func (pq *ProductQuery) loadPriceHistories(ctx context.Context, query *PriceHistoryQuery, nodes []*Product, init func(*Product), assign func(*Product, *PriceHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(pricehistory.FieldProductID)
	}
	query.Where(predicate.PriceHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.PriceHistoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
